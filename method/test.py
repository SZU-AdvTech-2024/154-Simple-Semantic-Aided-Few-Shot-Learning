...
from utils.utils import set_seed, Cosine_classifier, count_95acc, count_kacc, transform_val_cifar, transform_val, \
    transform_val_224, transform_val_224_cifar
...
if __name__ == '__main__':
    ...
    args.work_dir = './result/{}_{}_{}_{}_{}_{}'.format(args.backbone, args.dataset, args.mode, args.text_type, args.center,
                                               args.shot)

    log = loggers('./test/test_{}'.format(args.dataset))
    log.info(vars(args))
    set_seed(args.seed)

    if args.dataset == 'FC100':
        args.test = r'C:\workspace\code\PythonProject\SemFew\dataset\FC100\FC1001\test'
        test_dataset = ImageFolder(args.test,
                                   transform=transform_val_cifar if args.backbone == 'resnet' else transform_val_224_cifar)
    elif args.dataset == 'CIFAR-FS':
        args.test = r'C:\workspace\code\PythonProject\SemFew\dataset\CIFAR-FS\cifar-fs\test'
        test_dataset = ImageFolder(args.test,
                                   transform=transform_val_cifar if args.backbone == 'resnet' else transform_val_224_cifar)
    else:
